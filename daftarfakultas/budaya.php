...
$Inggris = "SELECT * FROM tbl_mhsiswa WHERE jurusan='Inggris'";
$tbl_Inggris = mysqli_query($conn, $Inggris);

$Jepang = "SELECT * FROM tbl_mhsiswa WHERE jurusan='Jepang'";
$tbl_Jepang = mysqli_query($conn, $Jepang);
        ...
        <div class="overflow-hidden">
          <table class="min-w-full text-left rounded text-sm font-light text-surface dark:text-black bg-slate-50">
            ...
            <tbody>
              ...
              if (mysqli_num_rows($tbl_Inggris) > 0) {
                // Loop melalui setiap baris data
                while ($row = mysqli_fetch_assoc($tbl_Inggris)) {
                  ?>
        ...
        <div class="overflow-hidden">
          <table class="min-w-full text-left rounded text-sm font-light text-surface dark:text-black bg-slate-50">
            ...
            <tbody>
              <?php
              // Periksa apakah ada baris data yang dihasilkan dari query
              if (mysqli_num_rows($tbl_Jepang) > 0) {
                // Loop melalui setiap baris data
                while ($row = mysqli_fetch_assoc($tbl_Jepang)) {
                  ?>
                  <tr class="cursor-pointer border-b border-neutral-200 bg-black/[0.02] dark:border-white/10 font-medium">
                    <td class="px-6 py-4 border-b border-gray-300">
                      <?php echo $row['nim']; ?>
