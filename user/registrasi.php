...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nama_mahasiswa) || empty($kelamin) || empty($status) || empty($jurusan) || empty($tgl_lahir) || empty($asal_sekolah) || empty($pekerjaan) || empty($alamat) || empty($kota) || empty($provinsi) || empty($telepon) || empty($email)) {
        ...
        $query = "INSERT INTO tbl_mhsiswa (nama_mahasiswa, jns_kelamin, status_mhs, jurusan, tgl_lahir, nim, lulusan_sekolah, pekerjaan, alamat,kota,provinsi,telp,email, tahun_ajaran) 
                  VALUES ('$nama_mahasiswa', '$kelamin', '$status','$jurusan','$tgl_lahir','$nim','$asal_sekolah','$pekerjaan','$alamat','$kota','$provinsi','$telepon','$email','$tahun_ajaran')";
        // Eksekusi query
...
<body class="bg-gray-300 ">
    <div class="dash px-[20px] py-10 bg-slate-400 ">
        <h2 class="font-bold text-[45px] rounded-md mb-6 text-center py-5   bg-red-600 text-slate-100">Data Pendaftar
        </h2>
